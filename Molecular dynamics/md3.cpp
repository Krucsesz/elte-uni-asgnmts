...
double rho = 0.02;         // density (number per unit volume)
...
void computeAccelerations() {
    ...
    for (int p = 0; p < nPairs; p++) {
        ...
        if (rSqdPair[p] < rCutOff*rCutOff) {
            ...
            U_pot += 4 * r6Inv * (r6Inv - 1);

            double f = 24*r2Inv*r6Inv*(2*r6Inv - 1);

            W_virial += f * rSqdPair[p];
...
int main() {
    ...
    for (int i = 0; i < 20000; i++) {
        ...
        double K_kin = 1.5 * (N - 1) * T_inst;
        double E_tot = (K_kin + U_pot) / N;
        double V = L * L * L;
        double P = (N * T_inst + W_virial / 3.0) / V;
        double Z = P / (rho * T_inst);
        
        if (i % 50 == 0)
            T += 0.005; // lassan hűtjük a rendszert
            rescaleVelocities();
    ...
    cout << "Heating sim complete..." << endl;
}
