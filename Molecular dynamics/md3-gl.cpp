...
double rCutOff = 10000;     // cut-off on Lennard-Jones potential and force
double rMax = 10000;        // maximum separation to include in pair list
...
void computeAccelerations() {
    ...
    for (int p = 0; p < nPairs; p++) {
        ...
        if (rSqdPair[p] < rCutOff*rCutOff) {
            double softening = 0.1; // ne legyenek végtelen erők
            double dist = sqrt(rSqdPair[p] + softening * softening);
            double f = -1.0 / (dist * dist * dist);
            for (int d = 0; d < 3; d++) {
...
void initPositions() {
    ...
    L = pow(N / rho, 1.0/3);
    for (int i = 0; i < N; i++) {
        r[3*i+0] = L/2.0 + (2 * rand() / double(RAND_MAX) - 1.0) * L/4.0;
        r[3*i+1] = L/2.0 + (2 * rand() / double(RAND_MAX) - 1.0) * L/4.0;
        r[3*i+2] = L/2.0 + (2 * rand() / double(RAND_MAX) - 1.0) * L/4.0;
    }
}
